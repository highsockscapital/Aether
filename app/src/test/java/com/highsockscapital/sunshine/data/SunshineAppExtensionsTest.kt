...
class SunshineAppExtensionsTest {
    ...
    fun disabledExtensionIdsBecomeStableLoadFilters() {
        ...
        assertEquals(
            setOf("/root/.sunshine/extensions/global-skills-mod"),
    ...
    fun migratesAndroidHostPathsToStableGuestPaths() {
        val hostPath = "/data/user/0/com.highsockscapital.sunshine/files/runtimes/alpine/rootfs" +
            "/root/.sunshine/extensions/pi-mcp-adapter"

        assertEquals(
            "import:sunshine:/root/.sunshine/extensions/pi-mcp-adapter",
            normalizeExtensionStateId("import:sunshine:$hostPath"),
        )
        assertEquals(
            setOf("/root/.sunshine/extensions/pi-mcp-adapter"),
            loadOptionsForIds(setOf("import:sunshine:$hostPath")).disabledExtensionPaths,
        )
    }

    @Test
    fun parsesExtensionSnapshotAndOrdersSlots() {
        val snapshot = parseSunshineAppExtensionSnapshot(
            JSONObject(
                """
                {
                  "api_version": 2,
