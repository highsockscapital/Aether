...
class AlpineRuntime(
    ...
    override val managedCommandsDirectory: String = "/root/.sunshine/bash-runs"
    ...
    suspend fun installPreinstalledExtensions(): Unit = withContext(Dispatchers.IO) {
        installAssetDirectoryRecursively("extensions", "/root/.sunshine/extensions")
    }
    ...
    private fun installAssetDirectoryRecursively(assetDir: String, guestTargetDir: String) {
        ...
        for (item in list) {
            if (
                assetDir == "extensions" &&
                guestPathToHostFile("/root/.sunshine/.removed-preinstalled-extensions/$item").existsNoFollow()
            ) {
                continue
            }
            val childAssetPath = "$assetDir/$item"
            val childGuestPath = "$guestTargetDir/$item"
            // Sunshine extensions are shared with user imports. Once a user owns a
    ...
    internal fun markPreinstalledExtensionRemoved(name: String) {
        ...
        val marker = guestPathToHostFile("/root/.sunshine/.removed-preinstalled-extensions/$safeName")
    ...
    internal fun clearPreinstalledExtensionRemoved(name: String) {
        val marker = guestPathToHostFile("/root/.sunshine/.removed-preinstalled-extensions/$name")
    ...
    override suspend fun replaceHostDirectories(
        ...
        val signatureFileName = ".sunshine-mirror-signature"
    ...
    private suspend fun executeCommand(
        ...
        val stdoutFile = File.createTempFile("sunshine-alpine-stdout", ".log", runtimeRoot)
        val stderrFile = File.createTempFile("sunshine-alpine-stderr", ".log", runtimeRoot)
    ...
    private fun streamProcessOutput(
        ...
        "sunshine-alpine-command-output",
    ).apply {
        isDaemon = true
        start()
    }
    ...
    private fun buildAlpineInteractiveProcess(): ProcessBuilder {
        ...
        return ProcessBuilder(prootCommand).apply {
            configureAlpineProcessEnvironment()
            environment()["PS1"] = "sunshine-alpine:\\w# "
    ...
    private fun buildAlpineProcessEnvironment(): Map<String, String> =
        buildMap {
            ...
            put("SUNSHINE_RUNTIME", "alpine")
            put("SUNSHINE_HOST_WORKSPACE", workspaceDir.absolutePath)
    ...
    private fun detectNetworkEnvironment(): ApkNetworkEnvironment {
        val countryCode = runCatching {
            ...
            try {
                ...
                connection.setRequestProperty("User-Agent", "Sunshine-Alpine-Network-Check")
