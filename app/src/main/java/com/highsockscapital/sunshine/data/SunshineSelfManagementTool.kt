...
class SunshineSelfManagementTool(
    ...
    fun toolDefinitions(): List<JSONObject> = listOf(
        buildSunshineToolDefinition(
            name = "sunshine_config_get",
            description = "Read Sunshine app configuration for general preferences, reliability, Skills, Termux, Agent Mode, scheduled tasks, and developer diagnostics. Provider and model configuration is intentionally omitted.",
        ...
        buildSunshineToolDefinition(
            name = "sunshine_config_set",
            description = "Modify allowed Sunshine settings. This tool cannot modify LLM provider, model, base URL, provider API keys, or default model selections.",
            properties = JSONObject().apply {
                put(
                    "category",
                    JSONObject().apply {
        ...
        buildSunshineToolDefinition(
            name = "sunshine_skill_manage",
            description = "List, install, enable, disable, or remove Sunshine Agent Skills. Remote installs accept GitHub repository URLs or zip URLs supported by Sunshine.",
            properties = JSONObject().apply {
                put(
                    "action",
                    JSONObject().apply {
        ...
        buildSunshineToolDefinition(
            name = "sunshine_termux_manage",
            description = "Inspect or repair Sunshine's Termux integration without using the bash tool. Root setup may trigger a system su request.",
            properties = JSONObject().apply {
                put(
                    "action",
                    JSONObject().apply {
        ...
        buildSunshineToolDefinition(
            name = "sunshine_agent_mode_manage",
        ...
        buildSunshineToolDefinition(
            name = "sunshine_scheduled_task_manage",
            description = "List, create, update, enable, disable, or remove Sunshine scheduled tasks. Scheduled tasks wake Sunshine at the next matching time and run the prompt as an automated Agent turn.",
            properties = JSONObject().apply {
                put(
                    "action",
                    JSONObject().apply {
        ...
        buildSunshineToolDefinition(
            name = "sunshine_extension_manage",
            description = "List or reload source-compatible Pi extensions for the current Sunshine session, or invoke a registered Pi extension command. Extensions are discovered from ~/.pi/agent/extensions, the workspace .pi/extensions directory, and ~/.sunshine/extensions. Custom Pi TUI components are not supported.",
            properties = JSONObject().apply {
                put(
                    "action",
                    JSONObject().apply {
        ...
        buildSunshineToolDefinition(
            name = "sunshine_developer_manage",
            description = "Read Sunshine developer diagnostics such as recent diagnostic events or last crash details. Sensitive values are redacted.",
            properties = JSONObject().apply {
                put(
                    "action",
                    JSONObject().apply {
    ...
    suspend fun execute(
        ...
        "sunshine_config_get" -> executeConfigGet(argumentsJson)
        "sunshine_config_set" -> executeConfigSet(argumentsJson)
        "sunshine_skill_manage" -> executeSkillManage(argumentsJson)
        "sunshine_termux_manage" -> executeTermuxManage(argumentsJson)
        "sunshine_agent_mode_manage" -> executeAgentModeManage(argumentsJson)
        "sunshine_scheduled_task_manage" -> executeScheduledTaskManage(argumentsJson)
        "sunshine_extension_manage" -> executeExtensionManage(argumentsJson)
        "sunshine_developer_manage" -> executeDeveloperManage(argumentsJson)
        else -> failure("Unknown Sunshine self-management tool '$toolName'.")
    }
    ...
    private suspend fun executeConfigGet(argumentsJson: String): String {
        ...
        return success(payload) {
            put("stdout", "Read ${categories.size} Sunshine configuration categories.")
        }
    }
    ...
    private suspend fun executeConfigSet(argumentsJson: String): String {
        ...
        return success(JSONObject().put(category, configCategoryJson(category, updated))) {
            put("stdout", "Updated Sunshine $category settings.")
        }
    }
    ...
    private suspend fun executeScheduledTaskManage(argumentsJson: String): String {
        ...
        return when (val action = arguments.optString("action").trim().lowercase(Locale.US)) {
            ...
            "create" -> {
                ...
                val task = scheduledTaskManager.upsertTask(
                    ScheduledTask(
                        ...
                        sessionId = optSunshineString(arguments, "session_id", "sessionId"),
            ...
            "update" -> {
                val taskId = optSunshineString(arguments, "task_id", "taskId")
                ...
                val updated = scheduledTaskManager.upsertTask(
                    existing.copy(
                        ...
                        schedule = schedule,
                        isEnabled = enabled,
                        sessionId = optSunshineString(arguments, "session_id", "sessionId").ifBlank { existing.sessionId },
                    )
                )
    ...
    private fun executeDeveloperManage(argumentsJson: String): String {
        ...
        return when (val action = arguments.optString("action").trim().lowercase(Locale.US)) {
            "read_diagnostics" -> {
                ...
                success(output) {
                    put("stdout", "Read Sunshine diagnostics.")
                }
            }
    ...
    private fun developerSummaryJson(): JSONObject =
        JSONObject()
            ...
            .put("tools", JSONArray(listOf("sunshine_developer_manage")))
                    ...
                    state.displays.forEach { display ->
                        put(
                            JSONObject()
                                ...
                                .put("is_sunshine_display", display.isSunshineDisplay)
                        )
                    }
                },
            )

    private fun buildSunshineToolDefinition(
        name: String,
        description: String,
