...
private const val SharedWorkspaceBaseDirectoryName = ".sunshine/workspace"
private const val PerSessionWorkspaceBaseDirectoryName = ".sunshine/workspaces"
...
class WorkspaceFileBridge(
    ...
    private fun runRootCommand(
        ...
        var stdout = ""
        var stderr = ""
        val stdoutThread = thread(start = true, name = "sunshine-root-read-stdout") {
            stdout = runCatching { process.inputStream.bufferedReader().readText() }
                .getOrDefault("")
        }
        val stderrThread = thread(start = true, name = "sunshine-root-read-stderr") {
    ...
    private fun buildInitWorkspaceUploadCommand(
        ...
        appendLine("tmp_b64=\"\${path}.sunshine-upload.b64\"")
        appendLine("tmp_path=\"\${path}.sunshine-tmp\"")
...
private class WorkspaceHttpUploadServer(
    ...
    private val worker = thread(
        name = "sunshine-workspace-upload",
        isDaemon = true,
        start = true,
    ) {
