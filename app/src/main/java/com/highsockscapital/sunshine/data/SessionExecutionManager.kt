...
import com.highsockscapital.sunshine.SunshineForegroundService
...
import com.highsockscapital.sunshine.ui.AttachmentKind
import com.highsockscapital.sunshine.ui.AssistantResponseBlock
import com.highsockscapital.sunshine.ui.ChatAttachment
import com.highsockscapital.sunshine.ui.ChatMessage
import com.highsockscapital.sunshine.ui.ChatSession
import com.highsockscapital.sunshine.ui.ChatToolInvocation
import com.highsockscapital.sunshine.ui.ChatUsageStatistics
import com.highsockscapital.sunshine.ui.MessageDisplayKind
import com.highsockscapital.sunshine.ui.MessageAuthor
import com.highsockscapital.sunshine.ui.ReasoningSummaryChunk
import com.highsockscapital.sunshine.ui.ReasoningTrace
...
class SessionExecutionManager(
    ...
    private val notificationController: SunshineNotificationController,
    ...
    private suspend fun executeTurn(
        ...
        val selfManagementTool = SunshineSelfManagementTool(
        ...
        return try {
            ...
            if (completion.piSessionId.isNotBlank() && completion.piSessionFile.isNotBlank()) {
                ...
                chatRepository.upsertAgentMessageRefs(
                    chatSessionId = handle.sessionId,
                    sunshineMessageIds = completion.appendedMessageIds,
    ...
    private fun buildRequestMessagesForChatMessage(
        ...
        val providerPayload = existingPayload ?: JSONObject().apply {
            put("piAssistantMessage", JSONObject().apply {
                put("role", "assistant")
                put("api", "sunshine")
                put("provider", settings.piProviderId.ifBlank { "sunshine" })
    ...
    private fun formatReasoningToolStatus(invocation: ChatToolInvocation): String {
        ...
        return when (invocation.toolName.lowercase()) {
            ...
            "sunshine_config_get" -> formatReasoningToolAction(
                isRunning = invocation.isRunning,
                runningVerb = "Reading",
                completedVerb = "Read",
                subject = formatSunshineReasoningCategories(arguments),
                fallback = "Sunshine settings",
            )

            "sunshine_config_set" -> formatReasoningToolAction(
                isRunning = invocation.isRunning,
            ...
            "sunshine_skill_manage" -> formatReasoningToolAction(
                isRunning = invocation.isRunning,
                runningVerb = sunshineSkillReasoningVerb(arguments, running = true),
                completedVerb = sunshineSkillReasoningVerb(arguments, running = false),
            ...
            "sunshine_mcp_manage" -> formatReasoningToolAction(
                isRunning = invocation.isRunning,
                runningVerb = sunshineMcpReasoningVerb(arguments, running = true),
                completedVerb = sunshineMcpReasoningVerb(arguments, running = false),
            ...
            "sunshine_termux_manage" -> when (arguments?.optString("action").orEmpty().lowercase()) {
            ...
            "sunshine_agent_mode_manage" -> when (arguments?.optString("action").orEmpty().lowercase()) {
            ...
            "sunshine_developer_manage" -> if (invocation.isRunning) "Reading Sunshine diagnostics" else "Read Sunshine diagnostics"
    ...
    private fun formatSunshineReasoningCategories(arguments: JSONObject?): String {
    ...
    private fun sunshineSkillReasoningVerb(
    ...
    private fun sunshineMcpReasoningVerb(
        arguments: JSONObject?,
        running: Boolean,
    ): String = when (arguments?.optString("action").orEmpty().lowercase()) {
