...
import com.highsockscapital.sunshine.data.StreamingStatus
...
class PiAgentRunner(
    ...
    private val toolExecutor: SunshineToolExecutor? = null,
    ...
    ): Result<SunshineAgentTurnResult> {
        onStreamingStatus(StreamingStatus("Thinking", "Sunshine is working on this turn."))
        ...
        return try {
            runCatchingPreservingCancellation {
                val resolvedSessionId = sessionId.ifBlank {
                    "sunshine-session-${System.currentTimeMillis()}"
                }
                ...
                val payload = JSONObject().apply {
                    ...
                    put(
                        "host_tools",
                        SunshineToolExecutor.hostToolDefinitions(
                ...
                coroutineScope {
                    ...
                    val eventHandler: suspend (String, JSONObject) -> Unit = { event, eventPayload ->
                        when (event) {
                            ...
                            "sunshine_host_call" -> dispatchSunshineHostCall(eventPayload, resolvedSessionId)
    ...
    private suspend fun handleHostToolRequest(
        ...
        workspaceDirectory: String,
        termuxWorkspaceDirectory: String,
        selfManagementTool: SunshineSelfManagementTool?,
        ...
        if (executor == null || !SunshineToolExecutor.supports(toolName)) {
                ...
                onProgress = { progress ->
                    bridge.sendHostToolProgress(
                        hostToolPayload(
                            ...
                            argumentsJson = argumentsJson,
                            rawOutput = progress,
                            isError = !SunshineToolExecutor.inferToolOutputOk(
                                SunshineToolExecutor.sanitizeToolOutputForConversation(toolName, progress),
                            ),
                        )
                    )
                },
    ...
    private suspend fun dispatchSunshineHostCall(
        payload: JSONObject,
        sessionId: String,
    ) {
        val method = payload.optString("method").trim()
        if (method != "sunshine_chrome_execute") {
            appExtensionManager?.handleAgentBridgeEvent("sunshine_host_call", payload)
                ?: bridge.sendSunshineHostResult(
                    callId = payload.optString("call_id"),
                    error = "The Sunshine UI host is unavailable.",
                )
            return
        }
...
private fun hostToolPayload(
    ...
    rawOutput: String,
    visibleOutput: String = SunshineToolExecutor.sanitizeToolOutputForConversation(toolName, rawOutput),
    isError: Boolean,
    systemPrompt: String = "",
