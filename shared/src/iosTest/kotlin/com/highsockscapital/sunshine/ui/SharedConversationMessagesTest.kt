...
import com.highsockscapital.sunshine.data.pi.SharedPiHostToolCall
...
class SharedConversationMessagesTest {
    ...
    fun browserToolsAreSeparatedFromRegularAssistantWork() {
        val browser = SharedChatToolInvocation(
            ...
            outputJson = """{"preview_path":"/workspace/.sunshine/browser-previews/browser-1.png","url":"https://example.com","title":"Example","width":390,"height":844,"stdout":"Opened page"}""",
            isRunning = false,
        )
        ...
        assertEquals("/workspace/.sunshine/browser-previews/browser-1.png", state.previewPath)
    ...
    fun workspaceLinksResolveFileUrisAndRelativeImages() {
        ...
        assertTrue(isSharedWorkspaceFileLink("sunshine-local-file://%2Fworkspace%2Freport.pdf"))
    ...
    fun nonMermaidFencesStayInMarkdownAndMermaidHtmlReportsHeightAndTap() {
        ...
        assertContains(html, "postSunshine('height:'")
        assertContains(html, "postSunshine('tap')")
    ...
    fun visibleReasoningModelsRouteNativeToolsIntoAReasoningTimeline() {
        val event = com.highsockscapital.sunshine.data.pi.SharedPiToolEvent(
            id = "read-1",
            name = "read",
