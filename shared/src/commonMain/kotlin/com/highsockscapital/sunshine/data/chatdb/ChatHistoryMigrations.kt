    ...
    override fun migrate(connection: SQLiteConnection) {
        ...
        connection.execSQL(
            ...
            CREATE TABLE IF NOT EXISTS `chat_agent_message_refs` (
                `chatSessionId` TEXT NOT NULL,
                `sunshineMessageId` TEXT NOT NULL,
                `piEntryId` TEXT NOT NULL,
                `ordinal` INTEGER NOT NULL,
                PRIMARY KEY(`chatSessionId`, `sunshineMessageId`, `piEntryId`),
                FOREIGN KEY(`chatSessionId`, `sunshineMessageId`) REFERENCES `chat_messages`(`sessionId`, `id`) ON UPDATE NO ACTION ON DELETE CASCADE
            )
            """.trimIndent(),
        )
        connection.execSQL(
        ...
        connection.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_chat_agent_message_refs_chatSessionId_sunshineMessageId` ON `chat_agent_message_refs` (`chatSessionId`, `sunshineMessageId`)",
        )
    }
}
