...
final class SunshineRuntimeTests: XCTestCase {
    private let host = SunshineRuntimeHost.shared
    ...
    func testAlpineFileManagerListsMovesAndRemovesGuestEntries() throws {
        try initializeRuntime()
        let directory = "/tmp/sunshine-file-manager-\(UUID().uuidString)"
    ...
    func testAlpineFileManagerImportsFilesAndFoldersWithoutOverwriting() throws {
        ...
        let sourceRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("sunshine-import-\(UUID().uuidString)", isDirectory: true)
        ...
        let destination = "/tmp/sunshine-import-destination-\(UUID().uuidString)"
    ...
    func testRetryResetAllowsMountedRuntimeToInitializeAgain() throws {
        ...
        let result = try run(
            "/bin/sh",
            arguments: ["-c", "node --version && test -f /root/.sunshine/pi-bridge/bridge.mjs"]
        )
        XCTAssertEqual(result.exitCode, 0, result.stderr)
    }
        ...
        let compiled = try run(
            ...
            arguments: [
                ...
                "command -v cc >/dev/null 2>&1 || " +
                    "apk add --no-cache build-base >/tmp/sunshine-build-base.log 2>&1; " +
    ...
    func testAlpineNodeBridgeTerminalCancellationAndStdioMcp() throws {
        ...
        let bridge = try run(
            "/usr/bin/node",
            arguments: ["/root/.sunshine/pi-bridge/bridge.mjs"],
        ...
        try writeGuestFile(
            "/workspace/e2e/sunshine-ios-npm/package.json",
            contents: #"{"name":"sunshine-ios-e2e","version":"1.0.0","description":"iOS npm extension E2E","sunshine":{"api":2,"extensions":["./index.mjs"]}}"#
        )
        try writeGuestFile(
            "/workspace/e2e/sunshine-ios-npm/index.mjs",
            contents: #"export default function(sunshine){sunshine.registerSurface('chat.composer.top',{tree:{type:'text',text:'ios-npm-extension-ok'}})}"#
        )
        let npmExtension = try runUntilStdoutContains(
            "/usr/bin/node",
            arguments: ["/root/.sunshine/pi-bridge/bridge.mjs"],
            stdin: Data(
                "{\"id\":\"npm-ios-e2e\",\"type\":\"install_extension_package\",\"payload\":{\"source\":\"npm:sunshine-ios-e2e@file:/workspace/e2e/sunshine-ios-npm\"}}\n".utf8
            ),
            marker: "\"id\":\"npm-ios-e2e\"",
            timeout: 180
        )
        XCTAssertTrue(npmExtension.stdout.contains("\"installed\":true"), npmExtension.stdout)
        XCTAssertTrue(npmExtension.stdout.contains("sunshine-ios-e2e"), npmExtension.stdout)
    ...
    func testMultithreadedForkKeepsParentMemoryPrivate() throws {
        ...
        let result = try run(
            ...
            arguments: [
                "-c",
                "command -v cc >/dev/null 2>&1 || apk add --no-cache build-base >/tmp/sunshine-build-base.log 2>&1; " +
