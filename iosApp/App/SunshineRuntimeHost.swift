...
final class SunshineRuntimeHost: NSObject, NativeRuntimeHost, UIDocumentPickerDelegate, PHPickerViewControllerDelegate, QLPreviewControllerDataSource, ASWebAuthenticationPresentationContextProviding {
    static let shared = SunshineRuntimeHost()

    private let runtime = SunshineISHRuntime.shared()
    private let operations = DispatchQueue(label: "com.highsockscapital.sunshine.runtime-host")
    ...
    private let backgroundExecution = SunshineBackgroundExecutionCoordinator.shared
    ...
    func refreshApkRepositoriesForCurrentNetwork() {
        ...
        request.setValue("Sunshine-Alpine-Network-Check", forHTTPHeaderField: "User-Agent")
    ...
    private func prepareInitializedRuntime() {
        do {
            ...
            try guestCreateDirectories("/opt/sunshine/chromium-deps")
            try guestBind(hostPath: chromeDependencies.path, guestPath: "/opt/sunshine/chromium-deps")
    ...
    func startProcess(
        ...
        let stdout: SunshineISHOutputBlock = { data in
            listener.onStdout(bytes: data.kotlinByteArray)
        }
        let stderr: SunshineISHOutputBlock = { data in
    ...
    func createTerminalView(listener: NativeTerminalViewListener) -> Any {
        let view = SunshineTerminalView(frame: .zero)
    ...
    func updateTerminalView(view: Any, bytes: KotlinByteArray) {
        (view as? SunshineTerminalView)?.feed(bytes.data)
    }

    func setTerminalDarkTheme(view: Any, darkTheme: Bool) {
        (view as? SunshineTerminalView)?.setDarkTheme(darkTheme)
    }

    func focusTerminalView(view: Any) {
        (view as? SunshineTerminalView)?.focus()
    }

    func sendTerminalKey(view: Any, key: String, controlDown: Bool, altDown: Bool) {
        (view as? SunshineTerminalView)?.sendKey(key, control: controlDown, alt: altDown)
    }

    func destroyTerminalView(view: Any) {
        (view as? SunshineTerminalView)?.cleanup()
    }
        ...
        operations.async { [self] in
            do {
                guard maximumBytes >= 0, maximumBytes <= Int64(Int.max) else {
                    throw NSError(
                        domain: "com.highsockscapital.sunshine.runtime-host",
                        code: 1,
    ...
    private func readPickedDirectory(_ root: URL, listener: NativePickedDirectoryListener) throws {
        ...
        guard let enumerator = FileManager.default.enumerator(
            ...
            throw NSError(
                domain: "com.highsockscapital.sunshine.file-picker",
                code: 2,
    ...
    private func temporaryFile(name: String, bytes: Data) -> URL? {
        ...
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("SunshineSharedFiles", isDirectory: true)
        do {
    ...
    private func alpineRuntimeRootURL() throws -> URL {
        if let container = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.com.highsockscapital.sunshine") {
            return container.appendingPathComponent("SunshineAlpine", isDirectory: true)
        }
        ...
        return applicationSupport.appendingPathComponent("SunshineAlpine", isDirectory: true)
    }
    ...
    private func alpineResetMarkerURL() throws -> URL {
        ...
        return applicationSupport.appendingPathComponent(".SunshineAlpineResetPending", isDirectory: false)
    }

    private func alpineSetupCompleteMarkerURL() throws -> URL {
        try alpineRuntimeRootURL()
            .appendingPathComponent(".sunshine-setup-complete", isDirectory: false)
    }
    ...
    private func purgePendingAlpineReset() throws {
        ...
        guard !runtime.isInitialized else {
            throw NSError(
                domain: "com.highsockscapital.sunshine.runtime-host",
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: "Restart Sunshine to finish resetting Alpine."]
            )
        }
        let root = try alpineRuntimeRootURL()
        if FileManager.default.fileExists(atPath: root.path) {
    ...
    private func installBridgeAsset() throws {
        try guestCreateDirectories("/root/.sunshine/pi-bridge")
        try installBridgeAsset(
            resource: "bridge",
    ...
    private func installBridgeAsset(
        ...
        let fingerprint = SHA256.hash(data: bytes)
            ...
            .joined()
        let bridgePath = "/root/.sunshine/pi-bridge/\(guestName)"
        let markerPath = "/root/.sunshine/pi-bridge/\(markerName)"
    ...
    private func installPreinstalledExtensions() throws {
        ...
        let guestBase = "/root/.sunshine/extensions"
        ...
        while let fileURL = enumerator.nextObject() as? URL {
            ...
            let removalMarker = "/root/.sunshine/.removed-preinstalled-extensions/\(topLevelName)"
...
private final class SunshineBackgroundExecutionCoordinator {
    static let shared = SunshineBackgroundExecutionCoordinator()
    ...
    private var taskIdentifierPrefix: String {
        "\(Bundle.main.bundleIdentifier ?? "com.highsockscapital.sunshine").agent"
    }
    ...
    func register() {
        onMainSync {
            ...
            if UIApplication.shared.backgroundRefreshStatus != .available {
                NSLog("Sunshine background refresh is unavailable (status: %ld)",
    ...
    private func ensureContinuedProcessingTask(name: String) {
        ...
        guard registered else {
            NSLog("Sunshine continued-processing task handler was not registered for %@", identifier)
            return
        }
        continuedTaskIdentifier = identifier
        NSLog("Sunshine continued-processing task handler registered for %@", identifier)
        ...
        do {
            ...
            NSLog("Sunshine continued-processing task submitted for %@", identifier)
        } catch {
            continuedTaskSubmitted = false
            continuedTaskIdentifier = nil
            let nsError = error as NSError
            NSLog("Sunshine continued-processing task submission failed (%@/%ld): %@",
    ...
    private func attach(_ task: BGContinuedProcessingTask) {
        NSLog("Sunshine continued-processing task started for %@", task.identifier)
