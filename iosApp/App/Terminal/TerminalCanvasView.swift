...
final class TerminalScrollContainerView: UIView {
    ...
    func setDarkTheme(_ darkTheme: Bool) {
        ...
        committedTailLength = 0
        committedHasConnector = false
        textView.textStorage.setAttributedString(NSAttributedString())
        lastRenderedVersion = UInt64.max
        terminalDidUpdate()
    }

    private func applyTheme() {
        let background = TerminalPalette.defaultBackground(darkTheme: darkTheme)
        backgroundColor = background
        scrollView.backgroundColor = background
        scrollView.indicatorStyle = darkTheme ? .white : .black
        textView.backgroundColor = background
        cursorOverlay.cursorColor = TerminalPalette.defaultForeground(darkTheme: darkTheme)
    ...
    private func buildRange(lines: [[TerminalCell]], trailingNewline: Bool) -> NSAttributedString {
        ...
        for (i, line) in lines.enumerated() {
            for cell in line {
                ...
                // Resolve colors
                var fg = TerminalPalette.resolve(
                    cell.foreground,
                    isForeground: true,
                    bold: isBold,
                    darkTheme: darkTheme
                )
                var bg = TerminalPalette.resolve(
                    cell.background,
                    isForeground: false,
                    darkTheme: darkTheme
                )
                ...
                if bg != TerminalPalette.defaultBackground(darkTheme: darkTheme) {
