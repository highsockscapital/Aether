...
static NSString *const SunshineISHErrorDomain = @"com.highsockscapital.sunshine.ish";

@interface SunshineISHProcess : NSObject
...
@property(nonatomic, copy) SunshineISHOutputBlock stdoutBlock;
@property(nonatomic, copy) SunshineISHOutputBlock stderrBlock;
@property(nonatomic, copy) SunshineISHExitBlock exitBlock;
@property(atomic) BOOL completed;
@property(atomic) BOOL exitDelivered;
@property(nonatomic) dispatch_group_t readerGroup;
@end

static int SunshineISHTTYInitialize(struct tty *tty) {
    return 0;
}

static int SunshineISHTTYWrite(struct tty *tty, const void *buffer, size_t length, bool blocking) {
    SunshineISHProcess *process = (__bridge SunshineISHProcess *)tty->data;
    if (!process || process.completed || length == 0) return (int)length;
    NSData *data = [NSData dataWithBytes:buffer length:length];
    SunshineISHOutputBlock output = process.stdoutBlock;
    dispatch_async(dispatch_get_main_queue(), ^{ output(data); });
    return (int)length;
}

static void SunshineISHTTYCleanup(struct tty *tty) {
    if (!tty->data) return;
    SunshineISHProcess *process = CFBridgingRelease(tty->data);
    tty->data = NULL;
    process.terminal = NULL;
}

static void SunshineISHReleaseTerminal(struct tty *tty) {
    if (!tty) return;
    SunshineISHTTYCleanup(tty);
    lock(&ttys_lock);
    tty_release(tty);
    unlock(&ttys_lock);
}

static struct tty_driver_ops SunshineISHTTYOperations = {
    .init = SunshineISHTTYInitialize,
    .write = SunshineISHTTYWrite,
    .cleanup = SunshineISHTTYCleanup,
};

static struct tty_driver SunshineISHPTYDriver = {.ops = &SunshineISHTTYOperations};

@implementation SunshineISHProcess
- (instancetype)init {
    self = [super init];
    if (self) {
        _stdinWrite = -1;
...
@interface SunshineISHRuntime ()
@property(nonatomic) dispatch_queue_t queue;
@property(nonatomic) dispatch_queue_t outputQueue;
@property(nonatomic) NSMutableDictionary<NSNumber *, SunshineISHProcess *> *processes;
...
static __weak SunshineISHRuntime *activeRuntime;
static void *SunshineISHQueueKey = &SunshineISHQueueKey;

static NSError *SunshineISHError(NSInteger code, NSString *message) {
    return [NSError errorWithDomain:SunshineISHErrorDomain
                               code:code
                           userInfo:@{NSLocalizedDescriptionKey: message}];
}

static NSError *SunshineISHNotInitializedError(void) {
    return SunshineISHError(_ENODEV, @"Alpine runtime is not initialized.");
}

static void SunshineISHProcessExited(struct task *task, int code) {
...
static void SunshineISHDie(const char *message) {
    NSLog(@"Sunshine iSH fatal error: %s", message ?: "unknown");
}

@implementation SunshineISHRuntime

+ (instancetype)sharedRuntime {
    static SunshineISHRuntime *runtime;
...
- (instancetype)initPrivate {
    self = [super init];
    if (self) {
        _queue = dispatch_queue_create("com.highsockscapital.sunshine.ish", DISPATCH_QUEUE_SERIAL);
        dispatch_queue_set_specific(_queue, SunshineISHQueueKey, SunshineISHQueueKey, NULL);
        _outputQueue = dispatch_queue_create("com.highsockscapital.sunshine.ish.output", DISPATCH_QUEUE_CONCURRENT);
...
- (instancetype)init {
    return [SunshineISHRuntime sharedRuntime];
}
...
- (void)initializeWithProgress:(SunshineISHProgressBlock)progress
    ...
    dispatch_async(self.queue, ^{
        ...
        if (bootError < 0) {
            error = SunshineISHError(bootError, [NSString stringWithFormat:@"Alpine boot failed (%d).", bootError]);
            dispatch_async(dispatch_get_main_queue(), ^{ completion(error); });
            return;
        }
...
- (NSURL *)prepareRootFileSystemWithProgress:(SunshineISHProgressBlock)progress
                                       error:(NSError **)error {
    NSURL *container = [NSFileManager.defaultManager containerURLForSecurityApplicationGroupIdentifier:@"group.com.highsockscapital.sunshine"];
    ...
    NSURL *rootURL = container ? [container URLByAppendingPathComponent:@"SunshineAlpine" isDirectory:YES] : (legacySupport ? [legacySupport URLByAppendingPathComponent:@"SunshineAlpine" isDirectory:YES] : nil);
    if (!rootURL) { if (error) *error = SunshineISHError(2, @"Storage unavailable."); return nil; }
    if (container && legacySupport) {
        NSURL *legacyRoot = [legacySupport URLByAppendingPathComponent:@"SunshineAlpine" isDirectory:YES];
    ...
    if (!archiveURL) {
        if (error) *error = SunshineISHError(1, @"Bundled Alpine rootfs is missing.");
        return nil;
    }
    ...
    if (!imported) {
        ...
        if (error) *error = SunshineISHError(importError.code, message);
...
- (int)bootRootAtURL:(NSURL *)rootURL {
    ...
    exit_hook = SunshineISHProcessExited;
    die_handler = SunshineISHDie;
#if !TARGET_OS_SIMULATOR
    NSString *socketPrefix = [NSTemporaryDirectory() stringByAppendingString:@"sunshine-ish-sock"];
...
- (int)startExecutable:(NSString *)executable
    ...
    [self performGuestOperation:^{
        ...
        SunshineISHProcess *process = [SunshineISHProcess new];
        ...
        if (pseudoTerminal) {
            struct tty *terminal = pty_open_fake(&SunshineISHPTYDriver);
            if (IS_ERR(terminal)) {
                current = savedCurrent;
                return;
            }
            ...
            if (error < 0) {
                SunshineISHReleaseTerminal(terminal);
                current = savedCurrent;
                return;
            }
        ...
        for (NSString *argument in allArguments) {
            ...
            if (argumentPosition + data.length + 2 >= sizeof(argumentBuffer)) {
                SunshineISHReleaseTerminal(process.terminal);
                current = savedCurrent;
                [self closePipe:stdinPipe];
                [self closePipe:stdoutPipe];
                [self closePipe:stderrPipe];
...
- (BOOL)performGuestOperation:(dispatch_block_t)operation {
    ...
    if (dispatch_get_specific(SunshineISHQueueKey)) {
        guarded();
    } else {
...
- (void)readDescriptor:(int)descriptor process:(SunshineISHProcess *)process block:(SunshineISHOutputBlock)block {
...
- (void)closeStdinForProcessId:(int)processId {
    @synchronized(self.processes) {
        SunshineISHProcess *process = self.processes[@(processId)];
        if (process.terminal) {
...
- (NSData *)readFile:(NSString *)path maximumBytes:(NSUInteger)maximumBytes error:(NSError **)error {
    ...
    BOOL performed = [self performGuestOperation:^{
        ...
        if (IS_ERR(fd)) {
            operationError = SunshineISHError(PTR_ERR(fd), @"Unable to open guest file.");
            return;
        }
        ...
        while (true) {
            ...
            if (count < 0) {
                operationError = SunshineISHError(count, @"Unable to read guest file.");
                break;
            }
            if (count == 0) break;
            if ((NSUInteger)count > maximumBytes - MIN(contents.length, maximumBytes)) {
                operationError = SunshineISHError(_EFBIG, @"Guest file exceeds the allowed size.");
                break;
            }
    ...
    if (!performed && !operationError) operationError = SunshineISHNotInitializedError();
...
- (BOOL)writeFile:(NSString *)path
       ...
       executable:(BOOL)executable
         progress:(SunshineISHFileWriteProgressBlock)progress
            error:(NSError **)error {
    __block NSError *operationError = nil;
    BOOL performed = [self performGuestOperation:^{
        struct fd *fd = generic_open(path.UTF8String, O_WRONLY_ | O_CREAT_ | O_TRUNC_, executable ? 0755 : 0644);
        if (IS_ERR(fd)) {
            operationError = SunshineISHError(PTR_ERR(fd), @"Unable to open guest file for writing.");
            return;
        }
        ...
        while (remaining > 0) {
            ...
            if (count <= 0) {
                operationError = SunshineISHError(count, @"Unable to write complete guest file.");
                break;
            }
            cursor += count;
    ...
    BOOL performed = [self performGuestOperation:^{
        ...
        for (NSString *component in components) {
            ...
            if (result < 0 && result != _EEXIST) {
                operationError = SunshineISHError(result, @"Unable to create guest directory.");
                break;
            }
        }
    }];
...
- (NSArray<NSDictionary<NSString *, id> *> *)listDirectory:(NSString *)path
    ...
    BOOL performed = [self performGuestOperation:^{
        ...
        if (IS_ERR(directory)) {
            operationError = SunshineISHError(PTR_ERR(directory), @"Unable to open guest directory.");
            return;
        }
        ...
        while ((result = directory->ops->readdir(directory, &entry)) > 0) {
            ...
            if (statResult < 0) {
                operationError = SunshineISHError(statResult, @"Unable to inspect guest directory entry.");
                break;
            }
        ...
        if (result < 0 && !operationError) {
            operationError = SunshineISHError(result, @"Unable to enumerate guest directory.");
        }
    }];
    if (!performed && !operationError) operationError = SunshineISHNotInitializedError();
    if (error) *error = operationError;
...
- (BOOL)movePath:(NSString *)sourcePath toPath:(NSString *)destinationPath error:(NSError **)error {
    ...
    if (result < 0 && error) *error = SunshineISHError(result, @"Unable to move guest path.");
    return result >= 0;
}
...
- (BOOL)removePath:(NSString *)path recursive:(BOOL)recursive error:(NSError **)error {
    ...
    if (result < 0 && error) *error = SunshineISHError(result, @"Unable to remove guest path.");
    return result >= 0;
}
...
- (BOOL)bindHostPath:(NSString *)hostPath guestPath:(NSString *)guestPath readOnly:(BOOL)readOnly error:(NSError **)error {
    if (![hostPath hasPrefix:NSHomeDirectory()]) {
        if (error) *error = SunshineISHError(1, @"Bind mount must remain inside the application sandbox.");
        return NO;
    }
    __block int result = _ENODEV;
    [self performGuestOperation:^{
    ...
    if (result < 0 && error) *error = SunshineISHError(result, @"Unable to create bind mount.");
    return result >= 0;
}
