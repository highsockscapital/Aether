/**
 * Sunshine Script Mod for pi-subagents.
 *
 * The Pi extension (src/index.ts) owns subagent execution and keeps running
 * inside Sunshine. Sunshine's TUI is absent there, so this mod re-homes the
 * TUI-facing parts on Sunshine Script API v2:
 *
...
type SunshineExtensionAPI = {
	ui: {
		...
		input(value: string, action: string, properties?: SunshineJsonObject): SunshineJsonObject;
	};
	...
	registerAction(id: string, handler: (payload: SunshineJsonObject, context?: SunshineRenderContext) => unknown | Promise<unknown>): () => void;
	registerToolTitle?(toolName: string, runningTitle: string, completedTitle: string, priority?: number): () => void;
	on?(event: string, handler: (payload: SunshineJsonObject) => unknown | Promise<unknown>): () => void;
	invalidate(): void;
...
const BRIDGE_KEY = Symbol.for("pi-subagents.sunshine-bridge");
const API_KEY = Symbol.for("pi-subagents.sunshine-api");
...
type SunshineBridgeState = {
	api: SunshineExtensionAPI;
	onBridge?: (bridge: SubagentsPiBridge) => void;
};

function readSunshineBridgeState(): SunshineBridgeState | undefined {
	return (globalThis as Record<PropertyKey, unknown>)[API_KEY] as SunshineBridgeState | undefined;
}
...
export function registerSubagentsBridge(bridge: SubagentsPiBridge): void {
	...
	const state = readSunshineBridgeState();
	if (!state) return;
...
/** Install or refresh the Sunshine API half of the bridge on Script Mod load. */
function attachSunshineApi(api: SunshineExtensionAPI, onBridge?: (bridge: SubagentsPiBridge) => void): void {
...
function settingsDefinition(): {
	...
	return {
		id: SETTINGS_PAGE_ID,
		title: "Subagents",
		subtitle: "Subagent concurrency, dispatch, persistence, and Sunshine UI",
		icon: "auto",
		order: 30,
		sections: [
			{
			...
			{
				id: "ui",
				title: "Sunshine and Pi UI",
				settings: [
					{ id: "widgetMode", label: "Widget", description: "Live Sunshine agent card visibility above the composer.", type: "select", options: widgetModeOptions() },
					{ id: "fleetView", label: "Fleet view", description: "Keep the TUI FleetView enabled for Pi CLI; Sunshine renders the same roster as tappable cards.", type: "toggle" },
...
function seedSettingsStorage(api: SunshineExtensionAPI): void {
...
async function applySettingAction(api: SunshineExtensionAPI, id: string, payload: SunshineJsonObject) {
...
function notificationCard(api: SunshineExtensionAPI, message: SunshineJsonObject) {
	const details = message.details && typeof message.details === "object"
		? message.details as SunshineJsonObject
		: message;
	...
	const others = Array.isArray(details.others) ? details.others.filter((entry): entry is SunshineJsonObject => !!entry && typeof entry === "object" && !Array.isArray(entry)) : [];
...
function resultCard(api: SunshineExtensionAPI, message: SunshineJsonObject) {
...
function conversationCard(api: SunshineExtensionAPI, message: SunshineJsonObject) {
	const text = messageText(message);
	return api.ui.card([
...
function liveAgentsCard(api: SunshineExtensionAPI): SunshineView {
	const bridge = readBridge();
...
function viewerOverlay(api: SunshineExtensionAPI, context: SunshineRenderContext): SunshineView {
	const bridge = readBridge();
...
function fallbackActivityCard(api: SunshineExtensionAPI, context: SunshineRenderContext): SunshineView {
...
export async function appendSunshineSubagentMessage(
	type: string,
...
async function runAgentAction(api: SunshineExtensionAPI, action: string, payload: SunshineJsonObject, context?: SunshineRenderContext): Promise<unknown> {
	const bridge = readBridge();
...
export const activateSunshine = async (api: SunshineExtensionAPI) => {
	...
	const registerAgentTypesPage = () => {
		...
		const typeSections: SunshineSettingsSection[] = typeSettings.length > 0
			? [{
				id: "types",
				title: "Agent types",
	...
	return () => {
		...
		if (readSunshineBridgeState()?.api === api) delete (globalThis as Record<PropertyKey, unknown>)[API_KEY];
	};
};
