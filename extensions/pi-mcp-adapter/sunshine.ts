...
import {
  attachMcpSunshineApi,
  readMcpSunshineBridge,
  type McpSunshineBridge,
  type McpSunshineServerSnapshot,
  type McpSunshineSnapshot,
} from "./sunshine-bridge.ts";

type SunshineJsonObject = Record<string, unknown>;
type SunshineView = SunshineJsonObject | SunshineView[] | string | null | undefined;
type SunshineRenderContext = SunshineJsonObject & { storage: SunshineJsonObject };
...
export interface SunshineExtensionAPI {
  ui: {
    node(type: string, properties?: SunshineJsonObject, children?: SunshineView[]): SunshineJsonObject;
    text(text: string, properties?: SunshineJsonObject): SunshineJsonObject;
    column(children: SunshineView[], properties?: SunshineJsonObject): SunshineJsonObject;
    row(children: SunshineView[], properties?: SunshineJsonObject): SunshineJsonObject;
    card(children: SunshineView[], properties?: SunshineJsonObject): SunshineJsonObject;
    button(label: string, action: string, properties?: SunshineJsonObject): SunshineJsonObject;
  };
  host: { invoke(method: string, args?: SunshineJsonObject): Promise<SunshineJsonObject> };
  storage: {
  ...
  messages: { append(type: string, payload?: SunshineJsonObject, text?: string): Promise<SunshineJsonObject> };
  registerSettings(definition: SunshineSettingsDefinition): () => void;
  registerAction(id: string, handler: (payload: SunshineJsonObject) => unknown | Promise<unknown>): () => void;
...
function readRawConfig(): SunshineJsonObject {
  const path = configPath();
  if (!existsSync(path)) return {};
  try {
    const parsed = parseJsonText(readFileSync(path, "utf8"));
    return parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed as SunshineJsonObject : {};
  } catch (error) {
    throw new Error(`Failed to read MCP config at ${path}: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
  }
}

function readServers(): Record<string, SunshineJsonObject> {
  const raw = readRawConfig();
  const servers = raw.mcpServers ?? raw["mcp-servers"] ?? {};
  if (!servers || typeof servers !== "object" || Array.isArray(servers)) return {};
  const result: Record<string, SunshineJsonObject> = {};
  for (const [name, entry] of Object.entries(servers)) {
    if (entry && typeof entry === "object" && !Array.isArray(entry)) result[name] = entry as SunshineJsonObject;
  }
  return result;
}

function writeServers(servers: Record<string, SunshineJsonObject>): void {
  const path = configPath();
  const raw = readRawConfig();
...
function stored(api: SunshineExtensionAPI, settingId: string, fallback: string): string {
...
function storedBoolean(api: SunshineExtensionAPI, settingId: string, fallback: boolean): boolean {
  const value = api.storage.get<boolean>(settingStorageKey(settingId))
    ?? api.storage.get<boolean>(settingId, fallback);
  return value === true;
}

function storeSetting(api: SunshineExtensionAPI, settingId: string, value: string | number | boolean): void {
  api.storage.set(settingId, value);
  api.storage.set(settingStorageKey(settingId), value);
}

function clearSetting(api: SunshineExtensionAPI, settingId: string): void {
  api.storage.delete(settingId);
  api.storage.delete(settingStorageKey(settingId));
}

function clearServerStorage(api: SunshineExtensionAPI, serverName: string): void {
...
function deriveTransport(entry: SunshineJsonObject): Transport {
...
function serverFieldValue(entry: SunshineJsonObject, field: string, fallback = ""): string {
  return asString(entry[field], fallback);
}

function authMode(entry: SunshineJsonObject): string {
...
function directToolsMode(entry: SunshineJsonObject): string {
  return booleanOrListMode(entry.directTools);
}

function booleanSelectValue(entry: SunshineJsonObject, field: string): string {
...
function oauthValue(entry: SunshineJsonObject, field: string, fallback = ""): string {
  const oauth = entry.oauth;
  if (!oauth || typeof oauth !== "object" || Array.isArray(oauth)) return fallback;
  return asString((oauth as SunshineJsonObject)[field], fallback);
}

function oauthSkipIssuer(entry: SunshineJsonObject): boolean {
  const oauth = entry.oauth;
  return Boolean(oauth && typeof oauth === "object" && !Array.isArray(oauth)
    ? (oauth as SunshineJsonObject).skipIssuerMetadataValidation === true
    : false);
}

function requestHeadersCommandValue(entry: SunshineJsonObject): string {
...
function text(id: string, labelText: string, description: string, value: string, extra: Partial<SunshineSettingDefinition> = {}): SunshineSettingDefinition {
  return { id, type: "text", label: labelText, description, default: value, ...extra };
}

function password(id: string, labelText: string, description: string, value: string): SunshineSettingDefinition {
  return { id, type: "password", label: labelText, description, default: value };
}

function textarea(id: string, labelText: string, description: string, value: string): SunshineSettingDefinition {
  return { id, type: "textarea", label: labelText, description, default: value };
}

function number(id: string, labelText: string, description: string, value: string): SunshineSettingDefinition {
  return { id, type: "number", label: labelText, description, default: value, min: 0 };
}

function toggle(id: string, labelText: string, description: string, value: boolean): SunshineSettingDefinition {
  return { id, type: "toggle", label: labelText, description, default: value };
}

function select(id: string, labelText: string, description: string, value: string, options: ReadonlyArray<{ value: string; label: string }>): SunshineSettingDefinition {
...
function button(id: string, labelText: string, action: string, args: SunshineJsonObject, description = "", tone: "primary" | "neutral" | "danger" = "neutral"): SunshineSettingDefinition {
  return { id, type: "button", label: labelText, description, action, args, tone };
}

function label(id: string, labelText: string, description: string): SunshineSettingDefinition {
  return { id, type: "label", label: labelText, description };
}

function link(id: string, labelText: string, url: string, description = ""): SunshineSettingDefinition {
...
function transportSettings(api: SunshineExtensionAPI, prefix: string, transport: Transport, entry: SunshineJsonObject): SunshineSettingDefinition[] {
  const value = (id: string, fallback: string) => stored(api, id, fallback);
  const settings: SunshineSettingDefinition[] = [];
  if (transport === "stdio") {
    settings.push(
    ...
    settings.push(
      ...
      textarea(`${prefix}oauthAuthorizationParams`, "OAuth authorization params", 'Optional JSON object of extra authorization URL parameters.', value(`${prefix}oauthAuthorizationParams`, formatRecord((entry.oauth as SunshineJsonObject | undefined)?.authorizationParams))),
...
function commonSettings(api: SunshineExtensionAPI, prefix: string, entry: SunshineJsonObject): SunshineSettingDefinition[] {
  const value = (id: string, fallback: string) => stored(api, id, fallback);
...
function serverCategory(api: SunshineExtensionAPI, name: string, entry: SunshineJsonObject, index: number, snapshot?: McpSunshineSnapshot): SunshineSettingsCategory {
  ...
  const sections: SunshineSettingsSection[] = [
...
function buildMainSections(api: SunshineExtensionAPI, servers: Record<string, SunshineJsonObject>, snapshot: McpSunshineSnapshot): SunshineSettingsSection[] {
  ...
  if (serverCount === 0) {
    ...
    const serverCards: SunshineSettingDefinition[] = serverNames.map((name) => {
      const entry = servers[name] as SunshineJsonObject;
      ...
      const actions: Array<{ label: string; action: string; args?: SunshineJsonObject; tone?: "primary" | "neutral" | "danger" }> = [
...
function oauthSection(api: SunshineExtensionAPI): SunshineSettingsSection | undefined {
  const pending = api.storage.get<SunshineJsonObject>(BRIDGE_OAUTH_KEY);
...
function statusLabel(status: McpSunshineServerSnapshot["status"]): string {
  return status.replaceAll("-", " ");
}

function emptySnapshot(): McpSunshineSnapshot {
  return {
    ready: false,
...
function parseOAuthObject(api: SunshineExtensionAPI, prefix: string, existing: SunshineJsonObject, force: boolean): SunshineJsonObject | undefined {
  ...
  const previous = existing.oauth && typeof existing.oauth === "object" && !Array.isArray(existing.oauth)
    ? existing.oauth as SunshineJsonObject
    : {};
  const next: SunshineJsonObject = { ...previous };
...
function applyText(entry: SunshineJsonObject, field: string, raw: unknown): void {
...
function applyInteger(entry: SunshineJsonObject, field: string, raw: unknown): void {
  const value = integerValue(asString(raw), true);
  if (value !== undefined) entry[field] = value;
  else delete entry[field];
}

function applyList(entry: SunshineJsonObject, field: string, raw: unknown, labelText: string): void {
  const value = parseList(asString(raw), labelText);
  if (value !== undefined) entry[field] = value;
  else delete entry[field];
}

function applyRecord(entry: SunshineJsonObject, field: string, raw: unknown, labelText: string): void {
  const value = parseRecord(asString(raw), labelText);
  if (value !== undefined) entry[field] = value;
  else delete entry[field];
}

function applyServerField(api: SunshineExtensionAPI, serverName: string, field: string, raw: unknown): boolean {
  const servers = readServers();
  const entry = servers[serverName];
  if (!entry) throw new Error(`Server "${serverName}" no longer exists.`);
  const next: SunshineJsonObject = { ...entry };
  const value = asString(raw);
  const prefix = `server:${serverName}:`;
...
function buildEntryFromForm(api: SunshineExtensionAPI, name: string, transport: Transport): SunshineJsonObject {
  const entry: SunshineJsonObject = {};
  const value = (id: string) => stored(api, id, "");
  if (transport === "stdio") {
...
function clearNewServerForm(api: SunshineExtensionAPI): void {
...
// ---------------------------------------------------------------------------
// Sunshine Script Mod entrypoint
// ---------------------------------------------------------------------------
...
export const activateSunshine = async (sunshine: SunshineExtensionAPI) => {
  for (const [toolName, runningTitle, completedTitle] of MCP_TOOL_TITLES) {
    sunshine.registerToolTitle?.(toolName, runningTitle, completedTitle, 200);
  }
  ...
  const syncDynamicToolTitles = () => {
    const toolNames = readMcpSunshineBridge()?.getSnapshot().toolNames ?? [];
    ...
    for (const toolName of current) {
      ...
      const cleanup = sunshine.registerToolTitle?.(toolName, `Calling ${toolName}`, `Called ${toolName}`, 100);
...
// ---------------------------------------------------------------------------
  const pageFingerprint = (servers: Record<string, SunshineJsonObject>, snapshot: McpSunshineSnapshot, pendingOAuth: boolean): string => JSON.stringify({
    path: snapshot.configPath,
    ready: snapshot.ready,
    pendingOAuth,
    servers: Object.keys(servers).sort().map((name) => [name, deriveTransport(servers[name] as SunshineJsonObject)]),
    ...
    inspections: Object.keys(servers).map((name) => [name, asString(sunshine.storage.get(`mcp:inspect:${name}`), "")]),
  });

  const registerSettingActions = (definition: { sections?: SunshineSettingsSection[]; categories?: SunshineSettingsCategory[] }) => {
    ...
    for (const section of sections) {
      for (const setting of section.settings) {
        ...
        seen.add(setting.id);
        sunshine.registerAction(`settings:${PAGE_ID}:${setting.id}`, async (payload) => {
          ...
          if (setting.id.startsWith("new_")) {
            storeSetting(sunshine, setting.id, current);
          ...
          if (setting.id.startsWith("server:")) {
            ...
            try {
              ...
              const changed = applyServerField(sunshine, serverName, field, current);
  ...
  const registerPage = (force: boolean) => {
    let servers: Record<string, SunshineJsonObject>;
    try {
      servers = readServers();
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      sunshine.notify(message, "error");
      servers = {};
    }
    const snapshot = readMcpSunshineBridge()?.getSnapshot() ?? emptySnapshot();
    const pendingOAuth = Boolean(sunshine.storage.get<SunshineJsonObject>(BRIDGE_OAUTH_KEY));
    const fingerprint = pageFingerprint(servers, snapshot, pendingOAuth);
    if (!force && fingerprint === lastFingerprint) return;
    lastFingerprint = fingerprint;

    const definition: SunshineSettingsDefinition = {
      id: PAGE_ID,
      title: "MCP Servers",
      ...
      sections: buildMainSections(sunshine, servers, snapshot),
      categories: [
        {
          id: "new-server",
          ...
          trailingIcon: "none",
          hidden: true,
          sections: [newServerSection(sunshine)],
        },
        ...Object.keys(servers).sort().map((name, index) => serverCategory(sunshine, name, servers[name] as SunshineJsonObject, index, snapshot)),
      ],
    };
    ...
    if (!seeded) {
      for (const section of [...(definition.sections ?? []), ...(definition.categories ?? []).flatMap((category) => category.sections)]) {
        for (const setting of section.settings) {
          ...
          const existing = sunshine.storage.get(settingStorageKey(setting.id)) ?? sunshine.storage.get(setting.id);
          const preserveDraft = setting.id.startsWith("new_") || setting.id === "oauth-input";
          if (!preserveDraft || existing === undefined || existing === null) {
            storeSetting(sunshine, setting.id, setting.default);
          }
        }
      }
      seeded = true;
    ...
    unregisterSettings?.();
    try {
      unregisterSettings = sunshine.registerSettings(definition);
    } catch {
      unregisterSettings = sunshine.registerSettings({
        id: definition.id,
  ...
  sunshine.registerAction("mcp:toggle-server", async (payload) => {
    try {
      ...
      sunshine.notify(`MCP server "${serverName}" ${checked ? "enabled" : "disabled"}.`, "info");
      scheduleRefresh(true);
      return { ok: true, name: serverName, enabled: checked };
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      sunshine.notify(`MCP: ${message}`, "error");
  ...
  const handleInspect = async (serverName: string, kind: "tools" | "resources" | "prompts") => {
    try {
      ...
      sunshine.storage.set(`mcp:inspect:${serverName}`, details);
      sunshine.notify(result.message, result.ok ? "info" : "warning");
      scheduleRefresh(true);
      return result;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      sunshine.storage.set(`mcp:inspect:${serverName}`, `Inspection failed: ${message}`);
      sunshine.notify(`MCP: ${message}`, "error");
      scheduleRefresh(true);
      return { ok: false, error: message };
    }
  ...
  sunshine.registerAction("mcp:inspect-tools", async (payload) => {
    const name = validateServerName(asString(payload.serverName));
    return handleInspect(name, "tools");
  });

  sunshine.registerAction("mcp:inspect-resources", async (payload) => {
    const name = validateServerName(asString(payload.serverName));
    return handleInspect(name, "resources");
  });

  sunshine.registerAction("mcp:inspect-prompts", async (payload) => {
    const name = validateServerName(asString(payload.serverName));
    return handleInspect(name, "prompts");
  });

  sunshine.registerAction("mcp:add-server", async () => {
    try {
      const name = validateServerName(stored(sunshine, "new_name", ""));
      const transportRaw = stored(sunshine, "new_transport", "stdio");
      ...
      const entry = buildEntryFromForm(sunshine, name, transport);
      writeServers({ ...servers, [name]: entry });
      clearNewServerForm(sunshine);
      sunshine.notify(`MCP server "${name}" added. Tap Reload MCP extension to connect.`, "info");
      scheduleRefresh(true);
      return { ok: true, name };
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  sunshine.registerAction("mcp:remove-server", async (payload) => {
    try {
      const name = validateServerName(asString(payload.serverName));
      const servers = readServers();
      ...
      clearServerStorage(sunshine, name);
      sunshine.notify(`MCP server "${name}" removed. Tap Reload MCP extension to apply.`, "info");
      scheduleRefresh(true);
      return { ok: true, name };
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  sunshine.registerAction("mcp:rename-server", async (payload) => {
    try {
      const name = validateServerName(asString(payload.serverName));
      const nextName = validateServerName(stored(sunshine, `server:${name}:renameTo`, ""));
      const servers = readServers();
      ...
      writeServers({ ...Object.fromEntries(Object.entries(servers).filter(([key]) => key !== name)), [nextName]: servers[name] as SunshineJsonObject });
      clearServerStorage(sunshine, name);
      sunshine.notify(`MCP server "${name}" renamed to "${nextName}". Tap Reload MCP extension to apply.`, "info");
      scheduleRefresh(true);
  ...
  const withBridge = async <T>(operation: (bridge: McpSunshineBridge) => Promise<T>, missingMessage: string): Promise<T> => {
    const bridge = readMcpSunshineBridge();
    if (!bridge) throw new Error(missingMessage);
    return operation(bridge);
  };

  sunshine.registerAction("mcp:reload", async () => {
    try {
      const result = await withBridge((bridge) => bridge.reload(), "The Pi MCP extension is not loaded yet.");
      if (result.ok) sunshine.notify("MCP extension reloading.", "info");
      else sunshine.notify(`MCP: ${result.message}`, "warning");
      return result;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      sunshine.notify(`MCP: ${message}`, "error");
  ...
  sunshine.registerAction("mcp:reconnect-all", async () => {
    try {
  ...
  sunshine.registerAction("mcp:reconnect-server", async (payload) => {
    try {
      const name = validateServerName(asString(payload.serverName));
  ...
  sunshine.registerAction("mcp:auth-start", async (payload) => {
    try {
      const name = validateServerName(asString(payload.serverName));
      ...
      if (result.ok && result.authorizationUrl) {
        sunshine.storage.set(BRIDGE_OAUTH_KEY, { serverName: name, authorizationUrl: result.authorizationUrl, startedAt: Date.now() });
        clearSetting(sunshine, "oauth-input");
        sunshine.notify("Authorization URL ready. Open it, approve access, then paste the callback URL back here.", "info");
      } else {
        sunshine.notify(result.message, result.ok ? "info" : "warning");
      }
      scheduleRefresh(true);
  ...
  sunshine.registerAction("mcp:oauth-complete", async (payload) => {
    try {
      const name = validateServerName(asString(payload.serverName));
      const input = stored(sunshine, "oauth-input", "").trim();
      ...
      sunshine.storage.delete(BRIDGE_OAUTH_KEY);
      clearSetting(sunshine, "oauth-input");
      sunshine.notify(result.message, result.ok ? "info" : "warning");
      scheduleRefresh(true);
      return result;
  ...
  sunshine.registerAction("mcp:oauth-logout", async (payload) => {
    try {
      const name = validateServerName(asString(payload.serverName));
