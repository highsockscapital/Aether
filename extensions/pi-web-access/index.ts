	...
	function startBackgroundFetch(urls: string[]): string | null {
		...
		fetchAllContent(urls, controller.signal)
			.then((fetched) => {
				...
				}, `Content fetched for ${ok}/${fetched.length} URLs. Full page content is ready.`);
			})
			.catch((err) => {
				if (!sessionActive || !pendingFetches.has(fetchId)) return;
				const message = err instanceof Error ? err.message : String(err);
	...
	pi.registerCommand("websearch", {
		...
		handler: async (args, ctx) => {
			...
			function sendFollowUpFromReturn(payload: ReturnType<typeof buildSearchReturn>) {
				...
				const completedText = typeof payload.details.summary === "object" && payload.details.summary
