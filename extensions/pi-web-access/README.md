...
**Zero Config** — Works out of the box with Exa MCP (no API key needed). If you're signed into Pi with a Codex subscription, OpenAI web search can reuse that auth. Add API keys for OpenAI, Brave, Parallel, TinyFish, Search1API, Searchinfinity, Querit, Tavily, SERPdive, Kagi, Ollama, SerpBase, Exa, Perplexity, or Gemini API for more control; configure a self-hosted SearXNG endpoint for private search; or opt into browser-cookie access for Gemini Web.

**Video Understanding** — Point it at a YouTube video or local screen recording and ask questions about what's on screen. Full transcripts, visual descriptions, and frame extraction at exact timestamps.

**Smart Fallbacks** — Every capability has a fallback chain. Search tries configured SearXNG first for local/private search, then OpenAI when suitable and available, Exa, Brave, Parallel, TinyFish, Search1API, Searchinfinity, Querit, Tavily, SERPdive, Kagi, Ollama, Perplexity, Gemini API, and Gemini Web when browser cookies are enabled. With no SearXNG configured, the existing zero-config order is unchanged. YouTube tries Gemini Web when enabled, then API, then Perplexity. Blocked pages try configured self-hosted Firecrawl first, then Jina Reader, TinyFish, Search1API, Querit, Kagi Extract, Ollama Web Fetch, Parallel, Bright Data Web Unlocker when a zone is configured, and Gemini extraction. Something always works.
...
Search the web via OpenAI, Brave, Parallel, TinyFish, Search1API, Searchinfinity, Querit, Tavily, SERPdive, Kagi, Ollama, AnySearch, xAI, Bright Data SERP, SerpBase, self-hosted SearXNG, Exa, Perplexity AI, or Gemini. Returns a synthesized answer with source citations.
...
| `provider` | Configured provider when omitted or set to `auto`; `all` searches every eligible provider except AnySearch, xAI, Bright Data, and SerpBase simultaneously; otherwise `openai`, `brave`, `parallel`, `tinyfish`, `search1api`, `searchinfinity`, `querit`, `tavily`, `serpdive`, `kagi`, `ollama`, `anysearch`, `xai`, `brightdata`, `serpbase`, `searxng`, `exa`, `perplexity`, or `gemini` (auto-selects when no provider or routing is configured; AnySearch, xAI, Bright Data, and SerpBase are explicit-only) |
...
fetch_content(url)
  ...
  → HTTP fetch → PDF? Gemini API → local text extraction, save to temp pi-web-pdf
               → HTML? Readability (+ declared Link/rel discovery) → RSC parser → Firecrawl (if configured, cache-only by default) → Jina Reader → TinyFish → Search1API → Querit → Kagi Extract → Ollama Web Fetch → Parallel → Bright Data Web Unlocker (if a zone is configured) → Gemini fallback
...
```json
{
  ...
  "braveApiKey": "BSA_...",
  "braveBaseUrl": "https://gateway.example.com/brave",
  "exaApiKey": "exa-...",
  "exaBaseUrl": "https://gateway.example.com/exa",
  "parallelApiKey": "...",
  "parallelBaseUrl": "https://gateway.example.com/parallel",
  "tinyfishApiKey": "sk-tinyfish-...",
  "tinyfishBaseUrl": "https://gateway.example.com/tinyfish",
  "search1apiApiKey": "...",
  "search1apiBaseUrl": "https://gateway.example.com/search1api",
  "searchinfinityApiKey": "...",
  "searchinfinityBaseUrl": "https://gateway.example.com/searchinfinity",
  "queritApiKey": "...",
  "queritBaseUrl": "https://gateway.example.com/querit",
  "tavilyApiKey": "tvly-...",
  "tavilyBaseUrl": "https://gateway.example.com/tavily",
  "serpdiveApiKey": "sd_live_...",
  "serpdiveBaseUrl": "https://gateway.example.com/serpdive",
  "serpdiveModel": "krill",
  "kagiApiKey": "$KAGI_API_KEY",
  "kagiBaseUrl": "https://gateway.example.com/kagi",
  "ollamaApiKey": "$OLLAMA_API_KEY",
  "ollamaBaseUrl": "https://gateway.example.com/ollama",
  "serpbaseApiKey": "$SERPBASE_API_KEY",
  "serpbaseBaseUrl": "https://gateway.example.com/serpbase",
  "brightdataApiKey": "$BRIGHTDATA_API_KEY",
  "brightdataBaseUrl": "https://gateway.example.com/brightdata",
  ...
  "perplexityApiKey": "pplx-...",
  "perplexityBaseUrl": "https://gateway.example.com/perplexity",
  "anysearchApiKey": "...",
  "anysearchBaseUrl": "https://gateway.example.com/anysearch",
  "xaiApiKey": "...",
  "xaiBaseUrl": "https://gateway.example.com/xai",
  "geminiApiKey": "AIza...",
...
Every keyed provider also accepts a literal `<provider>BaseUrl` override (`braveBaseUrl`, `parallelBaseUrl`, `tinyfishBaseUrl`, `search1apiBaseUrl`, `searchinfinityBaseUrl`, `queritBaseUrl`, `tavilyBaseUrl`, `serpdiveBaseUrl`, `kagiBaseUrl`, `ollamaBaseUrl`, `exaBaseUrl`, `perplexityBaseUrl`, `anysearchBaseUrl`, `xaiBaseUrl`, `brightdataBaseUrl`, `serpbaseBaseUrl`, plus the existing `openaiResponsesUrl`, `geminiBaseUrl`, `searxngBaseUrl`, and `firecrawlBaseUrl`). A value without a path (a bare origin such as `https://gateway.example.com`) mirrors every endpoint of that provider to the origin, preserving each default path; a value that includes a path replaces the primary endpoint URL verbatim (secondary extract/fetch/crawl endpoints are still mirrored to the override's origin). Providers with multiple hosts — TinyFish, Exa — route their API endpoints to the override while leaving non-API endpoints (Exa's zero-config MCP tool) untouched. Base URL overrides are literal config values and are not credential-interpolated.
...
classified. A billed `200` therefore always surfaces as an unreadable-response error, and can never be
reclassified as retryable and swallowed by `searchRouting.fallbackOn`.
