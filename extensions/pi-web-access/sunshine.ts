...
type SunshineSettingDefinition = {
	id: string;
	label: string;
	description?: string;
...
type SunshineSettingsSection = {
	id?: string;
	title?: string;
	description?: string;
	settings: SunshineSettingDefinition[];
};
type SunshineSettingsCategory = {
	id: string;
	title: string;
	subtitle?: string;
	icon?: string;
...
type SunshineMessageTypeDefinition = {
	type: string;
	title?: string;
	icon?: string;
	render: SunshineView | ((context: SunshineRenderContext & { message: SunshineJsonObject }) => SunshineView | Promise<SunshineView>);
};
type SunshineExtensionAPI = {
	...
	registerComposerMenuItem(definition: SunshineJsonObject & { id: string; title: string }): () => void;
	registerSurface(slot: string, definition: SunshineJsonObject & {
		render?: SunshineView | ((context: SunshineRenderContext) => SunshineView | Promise<SunshineView>);
	}): () => void;
...
const BRIDGE_KEY = Symbol.for("pi-web-access.sunshine-bridge");
...
type ConfigBinding = {
	setting: SunshineSettingDefinition;
	path: string[];
...
function providerSection(provider: string): SunshineSettingsSection & { bindings: ConfigBinding[] } {
...
const toolsSection: SunshineSettingsSection = {
	id: "tools",
...
/** Fallback layout for Sunshine builds that do not render page-level sections yet. */
const generalCategory: SunshineSettingsCategory = {
	id: "general",
	title: "Web search tools",
...
const extractionCategory: SunshineSettingsCategory = {
	id: "extraction",
...
const privacyCategory: SunshineSettingsCategory = {
	id: "privacy",
...
function categoriesForProvider(provider: string): SunshineSettingsCategory[] {
	return [
		{
			id: "provider",
			title: "Provider",
...
function writeConfig(config: Record<string, unknown>): void {
	...
	const temporaryPath = `${path}.${process.pid}.sunshine.tmp`;
...
function initialProvider(config: Record<string, unknown>, storage: SunshineJsonObject): string {
...
function messageText(message: SunshineJsonObject): string {
	...
	return message.content.map((part) => {
		...
		return typeof (part as SunshineJsonObject).text === "string" ? (part as SunshineJsonObject).text : "";
	}).join("\n");
}

function statusCard(api: SunshineExtensionAPI, title: string, message: SunshineJsonObject, tone = "neutral") {
	const text = messageText(message);
	const details = message.details && typeof message.details === "object" ? message.details as SunshineJsonObject : message;
	const chips = [
...
function messageTypes(api: SunshineExtensionAPI): SunshineMessageTypeDefinition[] {
	return [
...
export async function appendSunshineWebMessage(
	type: string,
...
export const activateSunshine = async (sunshine: SunshineExtensionAPI) => {
	(globalThis as Record<PropertyKey, unknown>)[BRIDGE_KEY] = { api: sunshine } satisfies WebAccessBridge;
	...
	try {
		...
		sunshine.host.invoke("app.notify", { message: `Web Access settings could not read the Pi config: ${message}` }).catch(() => {});
	}
	const storage = sunshine.storage.snapshot();
	...
	for (const binding of allBindings) {
		if (binding.sensitive) {
			...
			sunshine.storage.set(binding.setting.id, current);
			sunshine.storage.set(settingStorageKey(binding.setting.id), current);
			continue;
		}
		...
		sunshine.storage.set(binding.setting.id, initial);
	}
	...
	const registerSettingsPage = (providerValue: string) => {
		...
		try {
			sunshine.registerSettings(definition);
		} catch {
			// Older Sunshine builds only accept sections OR categories. Fall back
	...
	const registerBindingActions = () => {
		for (const binding of allBindings) {
			sunshine.registerAction(`settings:${SETTINGS_PAGE_ID}:${binding.setting.id}`, async (payload) => {
				...
				writeConfig(next);
				if (binding.sensitive) {
					sunshine.storage.set(binding.setting.id, value);
					sunshine.storage.set(settingStorageKey(binding.setting.id), value);
					await sunshine.host.invoke("app.notify", { message: "Credential or base URL updated. Reload the Pi extension to apply it." }).catch(() => {});
				...
				if (binding.setting.id === "searchRoutingProviders" && value !== "") {
					sunshine.storage.set("provider", "auto");
					sunshine.storage.set(settingStorageKey("provider"), "auto");
					registerSettingsPage("auto");
				}
				await sunshine.host.invoke("app.notify", { message: "Web Access setting saved. Reload the Pi extension to apply it." }).catch(() => {});
				return { setting: binding.setting.id, value };
			});
		}
	};
	...
	for (const definition of messageTypes(sunshine)) sunshine.registerMessageType(definition);

	sunshine.registerAction("dismiss-latest-activity", () => sunshine.storage.delete("latestActivity"));
	sunshine.registerAction("research-draft", async () => {
		await sunshine.host.invoke("app.appendDraftInput", { text: "Research this on the web with multiple independent sources: " });
	});
	...
	sunshine.registerSurface("chat.list.end", {
		...
		render: (context) => {
			...
			const activity = latest as SunshineJsonObject;
			const payload = activity.payload && typeof activity.payload === "object" ? activity.payload as SunshineJsonObject : {};
			...
			return sunshine.ui.column([
				statusCard(sunshine, title, { ...payload, text: activity.text }, type === "web-search-error" ? "error" : "neutral"),
				sunshine.ui.button("Dismiss", "dismiss-latest-activity", { tone: "neutral", icon: "close" }),
			], { spacing: 6 });
		},
	});
	...
	return () => {
		const bridge = (globalThis as Record<PropertyKey, unknown>)[BRIDGE_KEY] as WebAccessBridge | undefined;
		if (bridge?.api === sunshine) delete (globalThis as Record<PropertyKey, unknown>)[BRIDGE_KEY];
	};
};
